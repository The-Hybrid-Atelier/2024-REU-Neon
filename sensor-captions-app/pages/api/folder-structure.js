...
export default function handler(req, res) {
  try {
    ...
    const users = fs.readdirSync(basePath).filter(user => fs.lstatSync(path.join(basePath, user)).isDirectory());
    ...
    const folderStructure = users.map(user => {
      ...
      const bends = fs.readdirSync(userPath).filter(bend => fs.lstatSync(path.join(userPath, bend)).isDirectory());

      const userBends = bends.map(bend => {
        const bendPath = path.join(userPath, bend);
        const trials = fs.readdirSync(bendPath).filter(trial => fs.lstatSync(path.join(bendPath, trial)).isDirectory());

        const userBendTrials = trials.map(trial => {
          const trialPath = path.join(bendPath, trial);
          const videos = fs.readdirSync(trialPath).filter(file => file.endsWith('-video.mp4') || file.endsWith('-video.mov'));
